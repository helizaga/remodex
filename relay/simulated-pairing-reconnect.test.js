  ...
  await withServer(async ({ port, wss }) => {
    ...
    macSocket = new WebSocket(`ws://127.0.0.1:${port}/relay/${sessionId}`, {
      headers: {
        ...
        "x-notification-secret": "simulated-notification-secret",
    ...
    firstPhone.sendApplicationMessage(
      JSON.stringify({
        id: "request-1",
        method: "thread/list",
        params: {},
    ...
    const secondPhoneSocket = new WebSocket(`ws://127.0.0.1:${port}/relay/${sessionId}`, {
      headers: {
        ...
        "x-secure-handshake-mode": HANDSHAKE_MODE_TRUSTED_RECONNECT,
      },
    });
...
class SimulatedPhone {
  ...
  async completeHandshake({ handshakeMode, lastAppliedBridgeOutboundSeq }) {
    ...
    const phoneSignature = sign(null, phoneAuthTranscript, {
      key: {
        crv: "Ed25519",
...
async function resolveTrustedSessionEventually(port, { macDeviceId, phoneIdentity }) {
  let lastResult = null;
