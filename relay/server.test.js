...
test("health is minimal by default and detailed only when enabled", async () => {
  ...
  const detailed = await withServer(
    async ({ port }) => {
      const response = await fetch(`http://127.0.0.1:${port}/health`);
      return response.json();
    },
  ...
  await withServer(
    async ({ port }) => {
      const mac = new WebSocket(`ws://127.0.0.1:${port}/relay/session-health`, {
        headers: { "x-role": "mac", "x-notification-secret": "secret-health" },
      });
      await onceOpen(mac);
...
test("push routes are rate limited", async () => {
  const { body, status } = await withServer(
    async ({ port }) => {
      const response = await fetch(`http://127.0.0.1:${port}/v1/push/session/register-device`, {
        method: "POST",
        headers: { "content-type": "application/json" },
...
test("relay logs redact live session identifiers", async () => {
  ...
  try {
    await withServer(async ({ port }) => {
      ...
      const android = new WebSocket(`ws://127.0.0.1:${port}/relay/session-sensitive?role=android`);
...
test("trusted reconnect with matching phone headers can replace the live iphone socket", async () => {
  await withServer(async ({ port }) => {
    ...
    const replacementIphone = new WebSocket(
      `ws://127.0.0.1:${port}/relay/session-trusted-replace`,
      {
        headers: phoneHeaders(trustedPhone),
      }
    );
...
test("websocket relay forwards between mac and android on the base relay path", async () => {
  await withServer(async ({ port }) => {
    const mac = new WebSocket(`ws://127.0.0.1:${port}/relay/session-android-1`, {
      headers: { "x-role": "mac", "x-notification-secret": "secret-android" },
    });
...
async function resolveTrustedSessionEventually(
  port,
  { macDeviceId, phoneIdentity, noncePrefix, attempts = 10 }
) {
  let lastResult = null;
...
function makeTrustedResolveBody({ macDeviceId, phoneIdentity, nonce, timestamp }) {
  ...
  return {
    ...
    nonce,
    timestamp,
    signature: sign(null, transcript, {
      key: {
        crv: "Ed25519",
