...
const { createDesktopIpcActionFollower } = require("./desktop-ipc-action-follower");
...
const { readThreadTurnsListPageFromSessionJsonl } = require("./session-jsonl-history");
...
async function fetchAdaptiveThreadTurnsListForRelay(
  ...
  while (combinedTurns.length < requestedLimit) {
    ...
    if (
      measureSanitizedTurnsListResponseBytes(response, sanitizeForRelay) >= payloadSoftLimitBytes
    ) {
      response =
...
async function fetchSafeThreadTurnsListFallback(
  request,
  { fetchPage, now, sanitizeForRelay, payloadSoftLimitBytes }
) {
  const params = request?.params;
  const requestedLimit =
    Number.isInteger(params?.limit) && params.limit > 0
