  ...
  while (lineStart < raw.length) {
    ...
    if (entry?.type === "turn_context") {
      ...
      sessionTimeZone =
        ...
        sessionTimeZone;
      activeTurnId =
        normalizeString(payload?.turn_id) || normalizeString(payload?.turnId) || activeTurnId;
      if (activeTurnId) {
...
function addHistoryItemToTurn(turn, item) {
  ...
  if (isUserHistoryItem(item)) {
    const duplicateIndex = turn.items.findIndex((candidate) =>
      areDuplicateUserHistoryItems(candidate, item)
    );
    if (duplicateIndex !== -1) {
...
function hasStructuredUserHistoryContent(item) {
  ...
  return content
    ...
    .some((entry) => {
      ...
      return type === "skill" || type === "mention" || type === "image" || type === "inputimage";
    });
}
...
function isUserHistoryItem(item) {
  return (
    normalizeHistoryToken(item?.type) === "usermessage" ||
...
function historyUserItemText(item) {
  return (
    normalizeString(item?.text) || normalizeString(item?.message) || responseItemMessageText(item)
  );
}
...
function normalizeResponseItemForHistory(
  payload,
  lineNumber,
  { cwd = "", toolCallsByCallId = new Map() } = {}
) {
...
function normalizeToolOutputImageViewItemForHistory(
  payload,
  lineNumber,
...
function toolCallOutputContainsInlineImage(rawOutput) {
  const parsedOutput =
    typeof rawOutput === "string" ? safeParseJSON(rawOutput) || rawOutput : rawOutput;
